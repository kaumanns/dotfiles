...
# Based on https://keramida.wordpress.com/2013/03/28/controlling-the-keyboard-backlight-from-cli/
...
backlight_change()
{
    ...
    [uU][pP])
        ...
        if test "${current}" -lt "${max}" ; then
            value=$(( ${current} + ${change_value} ))
            if test "${value}" -gt "${max}"; then
              value=${max}
            fi
            backlight_set "${value}"
            notify-send -t 800 "Keyboard brightness set to ${value}"
